...
class yolov2(keras.callbacks.Callback,tf.keras.utils.Sequence):
    def __init__(self):
        ...
        self.datastring_height=68
        self.datastring_width=36
    ...
    def get_folder_filename(self,folder1):
      ...
      # for folder in folder1:
        ...
        #     print(i)
        #     j=a[0]+'.'+b[1]
        #     os.rename(folder+self.seg+i,folder+self.seg+j)
        #     print(j)
          # print(i)
      for folder in folder1:
        for i in os.listdir(folder):
    ...
    def make_model(self,succeed=True,yanyong=False):
        if (not succeed) or (self.prev_modelname==None) or (not os.path.exists(self.prev_modelname+'.json')):
            ...
            res_block_v2(x, input_filter, output_filter)
    ...
    def on_train_begin(self,logs=None):
      ...
      self.write_config(self.current_model+'-config.json')
    ...
    def begin_train(self,augmentation=True,succeed=False,yanyong=False):
      ...
      if augmentation:
        self.history=self.model.fit(self,initial_epoch=int(self.config_detail[0][1]/2),epochs=1000,verbose=0,
                                    validation_data=(self.validation_dataset_x,
                                                     {'detection':self.validation_dataset_y1,
                                                      'classification':self.validation_dataset_y2}),callbacks=[self])
      else:
        self.history=self.model.fit(self.train_dataset_x,{'detection':self.train_dataset_y1,'classification':self.train_dataset_y2},
                                    initial_epoch=int(self.config_detail[0][1]/2),epochs=2000,verbose=0,validation_data=
                                    (self.validation_dataset_x,{'detection':self.validation_dataset_y1,
                                                                'classification':self.validation_dataset_y2}),callbacks=[self])
      # print(self.hsitory)
...
# a.generate_dataset()
# a.kmeans(5)
a.begin_train(augmentation=False,succeed=False,yanyong=False)
# a.generate_dataset()
